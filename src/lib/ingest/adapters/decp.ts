  ...
  async *fetch(params: FetchParams): AsyncIterable<DecpRaw> {
    ...
    for (;;) {
      const where = encodeURIComponent(
        `lieuexecution_code="${departement}" AND lieuexecution_typecode="Code département" AND datenotification>=date'${depuis}'`,
      );
      ...
      for (const brut of body.results) {
        ...
        else throw new Error(`[decp] réponse inattendue : ${parsed.error.issues[0]?.message}`);
      }
      offset += limit;
